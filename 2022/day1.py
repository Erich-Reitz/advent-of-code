...
def part1(input: list[str]):
    ...
    return max([sum(string_utils.convert_list_of_strings_to_list_ints(cal_list)) for cal_list in divided_list])


def part2(input: list[str]): 
    divided_list = list_utils.split_list_by_value(input, '')
    cal_totals = [sum(string_utils.convert_list_of_strings_to_list_ints(cal_list)) for cal_list in divided_list]
